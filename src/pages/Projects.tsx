          ...
          {allProjects.map((project: any, index: number) => (
            ...
            >
              ...
              <div className="lg:col-span-7 space-y-8">
                <div>
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    {project.starProject && (
                      <span className="flex items-center gap-1 px-3 py-1 rounded-full bg-[var(--accent)]/10 text-[var(--accent)] text-xs font-bold uppercase tracking-widest">
                        <Zap size={12} /> Star Project
                      </span>
                    )}
                  </div>
                  <h2 className="text-3xl md:text-5xl font-bold mb-4">{project.name}</h2>
                  <p className="text-xl text-[var(--text)]/60 font-medium">{project.tagline}</p>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                  {project.problemStatement && (
                    <div className="p-6 rounded-2xl bg-[var(--text)]/5 border border-[var(--text)]/10">
                      ...
                      <p className="text-sm text-[var(--text)]/70">{project.problemStatement}</p>
                    </div>
                  )}
                  {project.businessImpact && (
                    <div className="p-6 rounded-2xl bg-[var(--secondary)]/5 border border-[var(--secondary)]/10">
                      <div className="flex items-center gap-2 mb-3 text-[var(--secondary)]">
                        <TrendingUp size={18} />
                        <h4 className="font-bold text-sm uppercase tracking-wider">Business Impact</h4>
                      </div>
                      <ul className="text-sm text-[var(--text)]/70 space-y-2">
                ...
                {project.architectureHighlights && (
                  <div className="p-8 rounded-3xl bg-[var(--primary)]/5 border border-[var(--primary)]/10">
                    ...
                    <ul className="grid md:grid-cols-2 gap-4">
                      {project.architectureHighlights.map((highlight: string, i: number) => (
                        <li key={i} className="flex items-start gap-3 text-sm text-[var(--text)]/70">
              ...
              <div className="lg:col-span-5 lg:sticky lg:top-32 space-y-8">
                <div className="p-8 rounded-3xl bg-[var(--text)] text-[var(--background)]">
                  <div className="mb-8">
                    <h4 className="text-[var(--primary)] text-xs font-bold uppercase tracking-widest mb-4">Tech Stack</h4>
                  ...
                  <div className="space-y-6">
                    <div>
                      <h4 className="text-[var(--primary)] text-xs font-bold uppercase tracking-widest mb-2">Company / Partnership</h4>
                      <p className="font-bold">{project.company || project.partnership}</p>
                    </div>
                    <div>
                      <h4 className="text-[var(--primary)] text-xs font-bold uppercase tracking-widest mb-2">My Role</h4>
                      <p className="font-bold">{project.role}</p>
                    </div>
                    <div>
                      <h4 className="text-[var(--primary)] text-xs font-bold uppercase tracking-widest mb-2">Timeline</h4>
                  ...
                  <div className="mt-10 flex gap-4">
                    {project.website && (
                      <a
                        ...
                        className="flex-1 flex items-center justify-center gap-2 bg-[var(--primary)] text-[var(--background)] py-3 rounded-xl font-bold transition-transform hover:scale-105"
                      >
                ...
                {project.impact && (
                  <div className="p-8 rounded-3xl border border-[var(--text)]/10">
                    ...
                    <div className="space-y-6">
                      {project.impact.map((imp: string, i: number) => (
                        <div key={i} className="flex gap-4 items-start">
                          ...
                          <p className="text-[var(--text)] font-medium leading-tight">{imp}</p>
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              {index !== allProjects.length - 1 && (
                <div className="lg:col-span-12 h-px bg-[var(--text)]/5 mt-20" />
              )}
            </motion.div>
          ))}
        </div>
